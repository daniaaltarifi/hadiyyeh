...
const handleProductTypeInsertion = (
  ...
  if (main_product_type === "Bag") {
    ...
    const insertWatchQuery = `INSERT INTO watches (ProductID, WatchTypeID, Available, before_price, after_price) VALUES (?, ?, ?, ?, ?)`;
    db.query(
      insertWatchQuery,
      [lastProductId, WatchTypeId, available, before_price, after_price],
      (err, watchResult) => {
        if (err) return res.status(500).json({ error: err.message });
...
const getProducts = (req, res) => {
  ...
  db.query(productQuery, [main_product_type], (err, results) => {
    ...
    console.log(results); // تسجيل النتائج لرؤية ما يتم استرجاعه
